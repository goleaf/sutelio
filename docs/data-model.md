...
- Fortify registration atomically persists the user, pending preference, personal workspace when no membership exists, owner pivot, and canonical task definitions. Mandatory onboarding completion applies the same idempotent application action and adds no project/task row. The populated-data-safe mandatory-onboarding migration reopens previously skipped incomplete preferences at Welcome with a new run identifier while retaining the old skip timestamp as audit until completion.
