...
- The emulator package sandbox created its own SQLite database in WAL mode. A consistent copy reports `integrity_check=ok`, zero foreign-key violation rows, and 38 applied migrations. The host database and any physical-device package were not changed.
- The build first exposed repository-local tooling, diagnostic state, and later ignored verification SQLite `-wal`/`-shm` files inside the bundle. Failing regressions reproduced both classes of defect, `config/nativephp.php` now excludes those inputs while preserving migrations, and `brand:native` deterministically removes the RequestInspector dependency plus NativePHP's verbose Android request/cookie/CSRF logging. The focused NativePHP/brand/localization suite passes 61 tests / 16,515 assertions, and both fresh complete Pest modes pass 967 tests / 40,456 assertions.
