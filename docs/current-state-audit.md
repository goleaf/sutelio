...
- The final ignored debug APK at `storage/app/native-build/sutelio-android-debug.apk` is 127,339,184 bytes with SHA-256 `e827ee3eed48f44151494be5541b6e93ec9a5e3983f57d9f0f868ed29f9b4408`. Its bytes match the final Gradle `app-debug.apk` exactly.
...
- A clean install on ARM64 Android 14 `emulator-5554` completed successfully. The final hardened artifact cold-launched in 3,690 ms, reached the Russian login through the first-run language flow, and relaunched in 1,333 ms with the locale persisted and no repeated first-run dialog. The Sutelio process remained resumed; process-scoped Logcat reported zero sensitive HTTP, PHP, WebView-console, fatal, and ANR matches, and DEX inspection found no RequestInspector package or sensitive logging literal.
- The emulator package sandbox created its own SQLite database in WAL mode. A consistent copy reports `integrity_check=ok`, zero foreign-key violation rows, and 38 applied migrations. The same checks passed after the exact verified APK was installed on the explicitly resolved Samsung SM-A920F Android 10 / API 29 device; the host database and unrelated/legacy phone packages remained unchanged.
