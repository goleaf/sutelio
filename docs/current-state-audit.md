...
| `php artisan database:health --json`                 | Passed: path, foreign keys, WAL, synchronous, busy timeout, cache/temp/checkpoint, quick check, and FK check healthy                                                                                      |
...
| --------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Web runtime           | Herd PHP 8.5.0; Laravel 13.25.0; 303 registered routes; application/database health passed                                                                                                                               |
...
| PHP verification      | Pest passed 627 tests / 8,958 assertions sequentially in 27.067 s and in parallel in 10.006 s; Pint and Larastan level 7 passed                                                                                          |
| Frontend verification | 13/13 Node tests, Vue type checking, ESLint, Prettier, and production build passed                                                                                                                                       |
| Build                 | 3,468 modules in 5.15 s; application CSS 154.98 kB / 24.23 kB gzip plus 2.57 kB / 0.48 kB font CSS; application entry 155.92 kB / 38.05 kB gzip                                                                          |
| Database/seeding      | 33 migrations from zero; repeat idempotent seed; 3 users, 1 workspace, 25 tasks; foreign-key check empty; all 17 models have factories                                                                                   |
...
| Runtime caches/smoke  | Config, routes, views, and optimize caches built; login 200 in 0.093211 s / 51,046 bytes; passkey discovery 200 in 0.034043 s / 117 bytes                                                                                |
| Browser               | Login, password confirmation, keyboard Inertia navigation, 20 desktop/mobile page checks, activity filters, reduced motion, dark media, and forced colors passed with zero current console/page errors and zero overflow |
