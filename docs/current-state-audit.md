...
The successor browser, complete-gate, Android-artifact, in-place rename, and physical-device phases are executed through Task 22. GitHub resolves the canonical public repository as `goleaf/sutelio`; local `origin`, the exact checkout `/Users/andrejprus/Herd/sutelio`, and the secure PHP 8.5 Herd site `https://sutelio.test` match that identity. Independent disposable Chrome DevTools and Playwright navigation returned HTTP 200 Sutelio login renders without console, failed dynamic-request, or horizontal-overflow evidence. The exact API 29 APK also installed and launched successfully on the connected Android 10 Samsung.

## Current Android Artifact And Emulator Evidence

- The final ignored debug APK at `storage/app/native-build/sutelio-android-debug.apk` is 127,462,553 bytes with SHA-256 `56be9a5ef9a28740868dc44d455ae2a2074d3a5ec75223d2412a8e642bfd251e`. Its bytes match the final Gradle, emulator-installed, and Samsung-installed `base.apk` files exactly.
- `aapt` and `apkanalyzer` report package `com.goleaf.sutelio`, label `Sutelio`, version code 1, debug version name, min SDK 29, target/compile SDK 36, ARM64 ABI, and launcher `com.nativephp.mobile.ui.MainActivity`. The external package identity remains separate from the intentional NativePHP/JNI namespace.
- `apksigner` verifies the debug v2 signature with certificate SHA-256 `1f0ff1db223c8ef90710774926028e80cf631e4739551306556b3191d8469198`; 16 KiB-aware `zipalign` verification passes. The nested Laravel archive retains migrations and compiled assets while excluding repository tooling, local diagnostic state, every local `database/*.sqlite*` file, and private key material. Its one cleaned runtime `.env` differs from the source file and has no nonempty sensitive value.
- A clean install on the task-owned ARM64 Android 14 emulator completed successfully. The exact artifact cold-launched in 1,726 ms, 1,440 ms, and 1,239 ms; phone/tablet, Russian localization, and animation-disabled frames retained the bounded static/motion contracts, `MainActivity` remained resumed, installed bytes matched, and process logs contained zero fatal/ANR/bitmap-decode matches. The exact AVD was removed without touching `BTChat_API_34`.
- After source publication and exact remote equality, ADB resolved one physical Samsung SM-A920F Android 10 / API 29 target. Only `com.goleaf.sutelio` was uninstalled, intentionally removing its old private database, cookies, preferences, and remembered-email ciphertext; package absence was proven before a clean install returned `Success`. The new first-install timestamp equals its update timestamp, no remembered-email file exists, installed bytes match the canonical artifact, cold launches completed in 2,632 ms, 2,559 ms, and 2,555 ms against the prior 4,909/4,903/4,870 ms baseline, fatal/ANR/bitmap-decode matches are zero, and `MainActivity` remains resumed. No other package or user data was targeted.
- The build first exposed repository-local tooling, diagnostic state, and later ignored verification SQLite `-wal`/`-shm` files inside the bundle. Failing regressions reproduced both classes of defect, `config/nativephp.php` now excludes those inputs while preserving migrations, and `brand:native` deterministically removes the RequestInspector dependency plus NativePHP's verbose Android request/cookie/CSRF logging. The focused NativePHP/brand/localization suite passes 61 tests / 16,515 assertions, and the fresh documentation-cleanup sequential and supported parallel Pest modes each pass 967 tests / 40,446 assertions.
...
This is the active modernization baseline. The superseded July audit files were removed during the 2026-08-18 documentation cleanup after their critical/high findings had been addressed in source and tests; their original content remains recoverable from Git history, while durable resolution evidence remains in `docs/progress.md` and the compliance matrix.
