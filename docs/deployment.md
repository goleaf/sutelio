...
The complete NativePHP 4.2 shell now packages every current mandatory-onboarding update and the explicit missing-preferences fallback in the independently inspected ignored debug deliverable at `storage/app/native-build/sutelio-android-debug.apk`. The current artifact is 127,338,768 bytes with SHA-256 `9a12f0cfd7c5276bfb3fe52ddce27bb29d2e25a833a3430a6040340c00684ad9`; it uses package `com.goleaf.sutelio`, label `Sutelio`, min SDK 29, target/compile SDK 36, ARM64 ABI, and one valid Android debug v2 signer. ZIP integrity and `zipalign` pass, and the nested Laravel bundle contains the mandatory-completion migration, replay-only exit route, stale-replay cleanup, explicit missing-preferences fallback, all 39 migrations, and built assets without the retired skip route, SQLite payloads, or sensitive environment keys. This exact artifact clean-installed on the isolated ARM64 Android 14 emulator; two cold launches returned `Status: ok`, `MainActivity` remained resumed, the first-run language dialog rendered, and the copied SQLite database passed integrity and foreign-key checks with all 39 migrations and zero users. After source/docs commit `14e16dc` reached `origin/main`, `adb install -r --no-streaming` installed the same hash on the single authorized Samsung SM-A920F Android 10 / API 29 while preserving package data. Physical cold launches completed in 2,538 ms and 2,358 ms; the final launch left `MainActivity` resumed as PID 7952, the installed `base.apk` matched the canonical size and SHA-256 exactly, a direct 675,840-byte SQLite copy returned integrity `ok`, zero foreign-key violations, all 39 migrations, and zero users, and process-scoped fatal/ANR and secret-value matches were zero. The first-run language dialog remains visible.
