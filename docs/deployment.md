...
The `deploy-production` workflow runs only after the existing `tests` workflow succeeds for the current `main` head in this repository. It checks out that exact SHA, rebuilds production Composer/npm dependencies, packages a secret-free immutable archive, and verifies the pinned SSH host identity. The workflow transfers both the archive and `deploy/activate-release.sh` from that tested commit, verifies both checksums on the server, and executes the staged activator directly; the provisioned `shared/bin/activate-release` remains a break-glass operator fallback rather than the CI entrypoint. GitHub environment `production` owns secret `DEPLOY_SSH_KEY` and variables `DEPLOY_HOST`, `DEPLOY_USER`, `DEPLOY_PATH`, and `DEPLOY_KNOWN_HOSTS`; its deployment branch policy admits only `main`. Never store the root credential, production `.env`, Laravel application key, or generated deploy private key in source or workflow output.
...
Expired-release removal runs only after the new release has passed its health gate and is best-effort. A permission or filesystem failure emits a warning containing only the immutable release SHA, leaves the healthy release active, and does not fail or roll back the deployment. The deploy script must not broaden privileges or ownership to force cleanup; an operator should inspect and remove the reported stale release through the existing aaPanel/root boundary.

Production mail defaults to Laravel's non-persistent `array` mailer until an approved SMTP provider and credentials exist. This prevents invitation and reset payloads from reaching either an external recipient or application logs, but it also means invitation, password-reset, and reminder delivery outside the process is not operational until mail is configured and tested.
