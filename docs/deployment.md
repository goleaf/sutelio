...
The complete NativePHP 4.2 shell now packages every current mandatory-onboarding update and the explicit missing-preferences fallback in the independently inspected ignored debug deliverable at `storage/app/native-build/sutelio-android-debug.apk`. The fresh candidate is 127,338,768 bytes with SHA-256 `9a12f0cfd7c5276bfb3fe52ddce27bb29d2e25a833a3430a6040340c00684ad9`; it uses package `com.goleaf.sutelio`, label `Sutelio`, min SDK 29, target/compile SDK 36, ARM64 ABI, and one valid Android debug v2 signer. ZIP integrity and `zipalign` pass, and the nested Laravel bundle contains the mandatory-completion migration, replay-only exit route, stale-replay cleanup, explicit missing-preferences fallback, all 39 migrations, and built assets without the retired skip route, SQLite payloads, or sensitive environment keys. This exact artifact clean-installed on the isolated ARM64 Android 14 emulator; two cold launches returned `Status: ok`, `MainActivity` remained resumed, the first-run language dialog rendered, and the copied SQLite database passed integrity and foreign-key checks with all 39 migrations and zero users. The reviewed source/docs commit must reach `origin/main` before this exact hash replaces the previous APK on the single authorized Samsung SM-A920F Android 10 / API 29; that data-preserving physical installation and its final database/log evidence remain pending.
