...
The complete NativePHP 4.2 shell now packages the mandatory-onboarding correction in the independently inspected ignored debug deliverable at `storage/app/native-build/sutelio-android-debug.apk`. The current artifact is 127,340,376 bytes with SHA-256 `10916bc861389d3b476d32bb95fdfff00528ce88652a1af4ee07096961819990`; it uses package `com.goleaf.sutelio`, label `Sutelio`, min SDK 29, target/compile SDK 36, ARM64 ABI, and one valid Android debug v2 signer. `zipalign` passed, and the nested Laravel bundle contains the mandatory-completion migration, replay-only exit route, stale-replay cleanup, all 39 migrations, and built assets without the retired skip route. This exact artifact clean-installed on the isolated ARM64 Android 14 emulator; two cold launches returned `Status: ok`, `MainActivity` remained resumed, the first-run language dialog rendered, and the copied SQLite database passed integrity and foreign-key checks with all 39 migrations and zero users. After implementation commit `e23dbb1` reached `origin/main`, `adb install -r --no-streaming` installed the same hash on the single authorized Samsung SM-A920F Android 10 / API 29 while preserving package data. Physical cold launches completed in 2,384 ms and 2,370 ms; the final launch left `MainActivity` resumed as PID 6085, a direct 675,840-byte SQLite copy returned integrity `ok`, zero foreign-key violations, all 39 migrations, and zero users, and process-scoped fatal/ANR matches were zero. The first-run language dialog remains visible.
