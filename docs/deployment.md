...
The complete NativePHP 4.2 shell refresh supersedes earlier Task 20 and Android 10 correction artifacts with the independently inspected ignored debug deliverable at `storage/app/native-build/sutelio-android-debug.apk`. The current artifact is 127,341,436 bytes with SHA-256 `b962f09d8d5a430664891e16e14c07c9864269960b9d7d1aff535738bae81231`; it uses package `com.goleaf.sutelio`, label `Sutelio`, min SDK 29, target/compile SDK 36, ARM64 ABI, and one valid Android debug v2 signer. `zipalign` passed, the hostless `sutelio` scheme is present, and the nested Laravel bundle contains all 38 migrations and built assets without repository tooling, local diagnostic state, SQLite/WAL/SHM files, private key material, or sensitive environment values. NativePHP's cleaned runtime `.env` remains present by design, differs from the source `.env`, and contains no application key, password, token, secret, mail, cloud, or Redis key. The final DEX contains neither the RequestInspector package nor sensitive diagnostic literals. This exact artifact clean-installed and cold-launched twice on the isolated ARM64 Android 14 emulator; both launches returned `Status: ok`, the second completed in 1,838 ms, the copied SQLite database passed integrity and foreign-key checks with all 38 migrations, and process-scoped fatal/ANR/secret-value matches were zero. After upgrade commit `f9dc768` reached `origin/main`, the same hash installed with data-preserving replacement on the explicitly resolved Samsung SM-A920F running Android 10 / API 29. Its two cold launches returned `Status: ok`; the final completed in 2,376 ms and left `MainActivity` resumed as PID `30789`. A read-only database copy returned integrity `ok`, zero foreign-key violations, and all 38 migrations; final process-scoped fatal/ANR/secret-value matches were zero. Authenticated Chrome and Playwright mobile checks provide the interaction evidence for sidebar dismissal.
