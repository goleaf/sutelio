...
- Identity review confirms one active product name, deterministic custom-ribbon/wordmark sources, canonical web/package/config/deep-link metadata, and EN/LT/RU proper-noun parity. The filesystem-based, case-insensitive legacy guard derives documentation from the canonical inventory and discovers safe text repository-wide, including hidden rules/workflows, arbitrary future roots, root configuration/locks, active decisions, public text, and first-party resources. It excludes only explicit historical evidence, intentional negative tests, Git/generated/vendor/dependency/storage/native output, binary assets/databases, credentials, secrets, and real environment files. A real `git archive` regression permits only the exact export-ignored canonical README and changelog when repository metadata is absent.
