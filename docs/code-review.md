...
- Identity review confirms one active product name, deterministic clean-S/wordmark sources, canonical web/package/config/deep-link metadata, and EN/LT/RU proper-noun parity. The active legacy guard derives its documentation scope from the canonical inventory and excludes only explicit historical evidence.
