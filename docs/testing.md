...
Automated brand tests prove deterministic source and generated-tree contracts, not a shipped artifact or human visual result. Browser verification must inspect title/metadata/favicon, shared/authentication logo composition, focus, forced colors, zoom, fixed-light behavior, and representative viewports. The current NativePHP 4.2 mandatory-onboarding artifact was independently inspected for manifest/resources/signature/alignment/archives, clean-installed on the Android 14 emulator, and installed with data-preserving replacement on the explicitly resolved Samsung SM-A920F running Android 10 / API 29 only after tracked source publication. Its cold launch/relaunch, first-run language dialog, SQLite sandbox integrity/foreign keys/39 migrations, resumed activity, and process-scoped logs pass on both Android targets. The deterministic brand test additionally guards removal of NativePHP request/header/cookie/response/CSRF value logging after every fresh native generation.
...
Critical identity, onboarding, workspace isolation, policy, token, backup/restore, recurrence/reminder, notification-link, and integrity branches require meaningful positive/negative coverage. When a coverage driver is available, measure application coverage and target at least 90% meaningful application-code coverage; do not add assertion-free tests or broad exclusions to inflate it. The fresh mandatory-onboarding sequential and supported parallel Pest suites each pass 973 tests / 40,584 assertions, and all 56 frontend tests pass. The fresh `php artisan test --coverage --compact` attempt exited before tests because the current Herd PHP 8.5 runtime has neither Xdebug nor PCOV; this remains tracked as `test-coverage-001`, and no coverage percentage is available or claimed.
