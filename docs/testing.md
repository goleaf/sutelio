...
- `BrandIdentityTest.php`: canonical metadata, active-file legacy identity exclusion, deterministic custom-ribbon geometry/raster/native outputs, browser/logo semantics, exact NativePHP 4.2 startup patching, build-prepared idempotency, and safe publication/rollback. `NativePhpMobileTest.php` separately protects runtime/package integration, including the external Sutelio IDs, unchanged internal Android namespace, localized animated splash resources, reduced-motion guard, and bitmap-free readiness handoff.
...
- `PageBasedRecordManagementTest.php`: dedicated workspace/project/task route authorization and page composition, foreign project rejection, record-overlay retirement, reusable in-flow confirmation semantics, and protected onboarding/global-loading exceptions.
- `FrontendDatePickerTest.php`: controlled open/close ownership, mobile fullscreen wrapper geometry contract, locale-aware weekend markers, all shared consumers, non-submitting controls, package/value/localization parity, and native date-input absence.
...
Native email assistance adds focused Pest contracts for successful/failed authentication persistence, bounded/corrupt/unavailable history behavior, encrypted/MAC-authenticated private-file storage, tamper rejection, private permissions, registration isolation, inline removal, plugin allowlisting, and zero account/contact permissions. Frontend tests own the asynchronous bridge-event adapter, cancellation, malformed payload, timeout cleanup, and concurrent-request rejection. Browser QA verifies responsive manual fallback with no native-only controls; Android compilation, artifact inspection, an isolated emulator flow, and the final physical-device update separately verify the plugin and persisted login history.
...
Use the existing browser automation/Boost logs rather than installing a duplicate framework. Critical smoke includes login, navigation, dashboard, tasks, dedicated project/task/workspace record pages, in-flow confirmation regions, settings/security/backup/import, validation and specialized dialogs/popovers at representative mobile/tablet/desktop widths, keyboard focus, the fixed light mode, reduced motion, long translations, no horizontal overflow, and no fresh console/page errors.
...
The page-based record/calendar smoke uses a product-created disposable account and completes mandatory onboarding without modifying onboarding source. It visits workspace create/edit/duplicate, project create/edit/duplicate, task create/detail, and a destructive workspace action; ordinary record pages must have one `h1`, zero visible record dialog, zero overflow, and confirmations must render as a focused `PageConfirmPanel` that restores its connected initiator on close. At 390×844 the date-picker content and its popper wrapper must both measure exactly from viewport origin to viewport edge, expose the mobile Close action and weekend markers, then disappear after a day choice without discarding a reminder time. Tablet/desktop retain bounded placement. Chrome DevTools and Playwright must both finish with clean current console/network evidence, and the disposable account must be deleted through product UI with a zero-residue read-only check.
