...
Tests verify locale key shape, placeholder parity, fallback, critical page rendering, localized validation and notifications, complete timezone grouping, browser detection, and locale/timezone-aware output. A missing human-quality translation must retain key parity and use the documented fallback honestly; it must never return a raw key in a critical flow.
