...
Tests verify locale key shape, non-empty values, placeholder parity, suspicious exact English copies, fallback, critical page rendering, active Laravel authentication/password/validation messages, typed client-error mappings, localized notifications, complete timezone grouping, browser detection, and locale/timezone-aware output. Locale-neutral templates, product names, technical identifiers, and examples require a narrow documented test allowlist. A missing human-quality translation must retain key parity and use the documented fallback honestly; it must never return a raw key in a critical flow.
