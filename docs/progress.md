...
- The existing localization/Native-route working-tree slice, its documentation, the configured local SQLite database, NativePHP artifacts, emulator, Samsung package, and physical-device state are outside this web deployment slice and remain untouched. Production outbound mail uses the safe log transport until approved SMTP credentials exist; external invitation, reset, and reminder delivery therefore remains a disclosed operational limitation rather than an invented success claim.
...
- A controlled lock-protected code rollback switched from `826be86` to the prior known-good `1812c30`, passed HTTPS and all ten database-health checks, then atomically restored `826be86` and passed both gates again without reversing schema or restoring a database file. The original localization/Native-route working slice, local configured database, NativePHP artifacts, emulator, Samsung application/data, and every unrelated dirty file remained untouched. External SMTP delivery is still intentionally disabled through the non-disclosing log transport; approved provider credentials and an immediate rotation of the separately supplied root credential remain operator actions rather than fabricated completion claims.
