...
The application bootstrap provides the authenticated `AppLayout`; onboarding pages instead resolve through a dedicated navigation-free layout and therefore retain exactly one `main` landmark without mounting the sidebar, authenticated header, workspace switcher, or application navigation. Onboarding step changes transfer focus to the connected step heading, validation transfers focus to one concise summary, and confirmation dialogs use the shared Reka-based focus lifecycle.
