...
Guided onboarding is a browser-only state machine immediately after authentication. Registration first uses `EnsureUserHasWorkspace` inside the same transaction to retain an authorized membership or create one localized personal workspace with owner membership and canonical task definitions; the web response selects it in the session. A narrow completion middleware then gates normal application routes, while onboarding lifecycle routes and signed invitation acceptance remain outside that gate. The onboarding query returns bounded user-authorized options; required skip/completion reuse the same idempotent workspace invariant, and explicit guided workspace/project/task creation records run-scoped request keys in `onboarding_operations`. Replay keeps the completion gate open and never deletes existing domain data; projects and tasks are never fabricated by bootstrap or skip.
